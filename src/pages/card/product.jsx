...
const ProductCard = ({ product }) => {
  ...
  return (
    <article className="flex flex-col bg-g_Text_White w-full h-fit">
      <div className="flex md:flex-row flex-col">
        <div className="flex items-center justify-center md:w-[12rem] w-full">
          <img
            src={product.img}
            alt=""
            className="object-contain md:w-[12rem] w-[8rem] h-full"
          />
        </div>
          ...
          <div className="flex flex-col items-start gap-1">
            ...
            <div className="flex flex-col text-[.8rem]">
              <div className="flex gap-2 justify-start">
                ...
                <label htmlFor={`gift${product.id}`} className="pb-1">
                  این یک هدیه است
                </label>
              </div>
              <div className="flex flex-row gap-1">
                {product.isAvailable ? (
                  <span className="flex w-fit text-[.7rem] text-green-600">
                    موجود
                  </span>
                ) : (
                  <span className="flex w-fit font-[500] text-red-600">
                    نا موجود
                  </span>
                )}
                {product.remaining ? (
                  <div className="flex gap-1 pt-[.1rem] text-red-600 -mt-[.2rem]">
                    {product.remaining}_ عدد در انبار باقی مانده
                  </div>
                ) : (
                  ""
                )}
          ...
          <div className="flex flex-row flex-wrap items-center pt-2">
            <div className="relative flex items-center justify-center ">
              ...
              <label
                ...
                htmlFor={product.id}
                className="absolute left-0 top-0 flex items-center justify-center font-[500] text-[.8rem] w-[73px] h-[30px] px-1 pb-1 rounded-[15px] bg-g_Background_White_Shop shadow-3xl"
              >
                : تعداد
              </label>
            </div>
      ...
      {product.isAvailable ? (
        <div className="flex justify-start w-full pb-4 pt-1 pr-4 border-t-2">
          هزینه کل :{" "}
          {(
